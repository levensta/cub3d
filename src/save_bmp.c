...
/*   Updated: 2021/02/13 21:14:03 by levensta         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

static void	bmp_header(t_all *cub, int fd)
{
	int		t;
	short	d;
...
static void	image_header(t_all *cub, int fd)
{
	int		t;
	short	d;
...
int			save_bmp(t_all *cub)
{
	...
	if ((fd = open("screenshot.bmp", O_RDWR | O_CREAT | O_TRUNC, 00600 |\
	00060 | 00006)) < 0)
